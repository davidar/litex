#!/usr/bin/env python3
# https://github.com/bl0x/learn-fpga-amaranth/blob/main/tools/riscv_assembler.py
import re
...
JInstructions = [("JAL", 0b1101111), ("JALR", 0b1100111, 0b000)]
...
UInstructions = [("LUI", 0b0110111), ("AUIPC", 0b0010111)]
...
SInstructions = [("SB", 0b000), ("SH", 0b001), ("SW", 0b010)]
...
class RiscvAssembler:
    ...
    def encodeS(self, imm, rs2, rs1, f3, op):
        imm25 = (imm >> 5) & 0x7F
        imm7 = imm & 0x1F
        return (imm25 << 25) | (rs2 << 20) | (rs1 << 15) | (f3 << 12) | (imm7 << 7) | op
    ...
    def unravelPseudoOps(self, instruction):
        ...
        if op == "NOP":
            ...
            if imm == 0:
                ...
                instr.append(self.iFromLine("ADDI {}, zero, {}".format(rd, imm)))
            else:
                ...
                if imm12 != 0:
                    instr.append(
                        self.iFromLine("ADDI {}, {}, {}".format(rd, rd, imm12))
                    )
        elif op == "CALL":
            ...
            instr.append(self.iFromLine("BLT   {}, {}, {}".format(rs2, rs1, ref)))
        else:
    ...
    def encode(self, instruction):
        ...
        if self.pc in self.pseudos:
            print(
                "  psu@pc=0x{:03x}={} -> {}".format(
                    self.pc, self.pc, self.pseudos[self.pc]
                )
            )
        print("  enc@pc=0x{:03x} {} -> 0b{:032b}".format(self.pc, instruction, encoded))
        self.pc += 4
        return encoded
    ...
    def iFromLine(self, line):
        ...
        if " " not in line:
            ...
            op, rest = [x.strip() for x in (line.split(" ", maxsplit=1))]
